App.Scorecard = {
    ...
    createListeners: function() {
        $('#scorecardModalSaveButton').on('click', function(event) {
            App.Scorecard.save(event);
        });
    },
    save: function(event) {
        ...
        if (name.trim() === '') {
            ...
            alert('Please enter a negotiation name.');
            return;
        }

        if (type === 'Select a negotiation type below..') {
            valid = false;
            event.preventDefault();
            event.stopImmediatePropagation();
            alert('Please select a negotiation type.');
            return;
        }

        if (agreement !== 'Yes' && agreement !== 'No' && agreement !== 'Not Yet' ) {
            valid = false;
            event.preventDefault();
            event.stopImmediatePropagation();
            alert('Please select a negotiation agreement.');
            return;
        }
